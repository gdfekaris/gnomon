...
**Inbox & proposals**: inbox captures, "process inbox with AI" action (with several captures waiting, the curator ticks the ones to file now and the rest keep waiting), ratify/reject flows built on commit history with the mobile review view from §9 decision 3. The Proposals screen has two parts: the reserve proposals a filing raises, as a triage list (keep or drop, singly or several at once in one commit, with a near-duplicate hint), then the proposals for each set from Relate and Derive, as cards with the context to judge them (the target principle's grounds or current wording, what the set holds); one search over both.

**Principle sets**: list of sets with ordinal and sub-name; new set, rename, reorder (drag), delete with confirmation and a dangling-reference report; per-set description editor (the `_set.md` body, which is also the set's framing instruction to the model); within a set, drag-reorder of principles. The screen is also the view of every principle: one search over all of them, and a final "In reserve" section for principles held but not in force, newest first or A–Z, with tag chips and pages of fifty; a reserve principle is placed into a set in one tap, a set's principle is kept in reserve instead of deleted in one tap, and a new principle can be written straight into the reserve.
