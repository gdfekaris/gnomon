<script lang="ts">
  // Capture — spec §10.4, US-1. The default screen: paste target, note,
  // attach-a-file, save. One commit; under fifteen seconds end to end.
  import { AttachmentTooLargeError, ATTACHMENT_LIMIT_BYTES } from '@gnomon/storage';
  import { brain } from '../lib/services/index';
  import { captureToInbox, type CaptureResult } from '../lib/services/capture';
  ...
  async function save() {
    ...
    try {
      ...
      saved = await captureToInbox(brain, { text, note, ...(attachment ? { attachment } : {}) });
      text = '';
      note = '';
      files = null;
...
{:else}
  ...
  <form onsubmit={(e) => { e.preventDefault(); void save(); }}>
    ...
    <button type="submit" disabled={saving || text.trim() === ''} data-testid="capture-save">{saving ? 'Saving…' : 'Save to inbox'}</button>
  </form>
...
<style>
  ...
  @media (prefers-color-scheme: dark) { .ok { background: #14532d; color: #dcfce7; } }
</style>
