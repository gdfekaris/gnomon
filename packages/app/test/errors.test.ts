...
describe('describeError (spec §14): one plain sentence per failure', () => {
  const cases: Array<[unknown, RegExp]> = [
    ...
    [new ReferenceError("Can't find variable: WebAssembly"), /without WebAssembly.*Lockdown Mode/],
    [new ReferenceError('WebAssembly is not defined'), /without WebAssembly/],
    [new Error('WebAssembly compile failed'), /^WebAssembly compile failed$/],
