...
{:else}
  ...
  {#if prefill}
    <div class="from" role="status" data-testid="from-proposal">
      ...
      <p class="hint">{prefill.kind === 'principle' ? 'The draft below is the proposal\'s wording, with its source as a ground. Save it as it is, or put it in your own words.' : prefill.kind === 'link' ? 'This proposes a ground: pick the source below and add it to this principle\'s grounds if you agree. The proposal is only a suggestion.' : 'Apply what you agree with by hand; the proposal is only a suggestion.'}</p>
    </div>
  {/if}
  {#if kind === 'principle'}
