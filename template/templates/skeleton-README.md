# Second Brain — structural scaffolding

A commonplace book that an AI can reason from.

This repo is the empty scaffolding only: no sources, no notes, no
principles. You supply those. There is nothing to install and nothing to
build — the system *is* the layout plus `AGENTS.md`, an operating manual
...
MIT — see `LICENSE`. It covers this scaffolding: the manual, the
templates, the commands, the layout. Whatever you file into it is yours.

## Starting out

Delete this README. Write one principle in `principles/` and list it under
**In force** in `principles/_index.md` — nothing loads as a premise until
you do. Then drop something in `inbox/` and run `/file-inbox`.

It will feel pointless early, and it is: one source and no principles is a
folder, not a brain. Capture and filing pay off at scale; `/reason` pays
off the moment you have two principles that disagree with each other.
